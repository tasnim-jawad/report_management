...
class ReportInfoController extends Controller
{
    public function all()
    {
        ...
        $query = ReportInfo::where('status', $status)->orderBy($orderBy, $orderByType);
        ...
        if (request()->has('search_key')) {
            ...
            $query->where(function ($q) use ($key) {
                ...
                ->orWhere('org_type', '%' . $key . '%')
                ->orWhere('org_type_id', '%' . $key . '%')
                ->orWhere('responsibility_id', '%' . $key . '%')
                ->orWhere('responsibility_name', '%' . $key . '%')
                ->orWhere('month_year', '%' . $key . '%')
                ->orWhere('report_type', '%' . $key . '%');
    ...
    public function store()
    {
        ...
        $data->responsibility_name = request()->responsibility_name;
    ...
    {
        $data = ReportInfo::find(request()->id);
        if (!$data) {
            return response()->json([
                'err_message' => 'validation error',
    ...
    {
        $validator = Validator::make(request()->all(), [
            'id' => ['required', 'exists:report_infos,id'],
        ]);
