...
class WardBmExpenseCategoryController extends Controller
{
    public function index(){
        $datas = WardBmExpenseCategory::where('status', 1)
                                ->orderBy('id','ASC')
                                ->get();
        return response([
            'data' => $datas,
    ...
    public function all()
    {
        ...
        $query = WardBmExpenseCategory::where('status', $status)->orderBy($orderBy, $orderByType);
    ...
    {
        $datas = WardBmExpenseCategory::where('parent_id', 0)
                                    ->orderBy('id', 'asc')
                                    ->where('status', 1)
                                    ->get();
    ...
    {
        $data = WardBmExpenseCategory::find(request()->id);
        if (!$data) {
            return response()->json([
                'err_message' => 'validation error',
