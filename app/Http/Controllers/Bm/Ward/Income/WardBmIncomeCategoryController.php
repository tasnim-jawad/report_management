...
class WardBmIncomeCategoryController extends Controller
{
    public function all()
    {
        ...
        $query = WardBmIncomeCategory::where('status', $status)->orderBy($orderBy, $orderByType);
    ...
    {
        $data = WardBmIncomeCategory::find(request()->id);
        if (!$data) {
            return response()->json([
                'err_message' => 'validation error',
