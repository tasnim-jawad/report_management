            ...
            <form action="" @submit.prevent="set_responsibility">
                ...
                <div class="d-flex flex-wrap gap-2 mb-2 align-items-center" v-for="(field, index) in fields1" :key="index">
                    ...
                    <div class="form_input" v-else-if="field.field_type == 'select' && field.name == 'ward_id'">
                        <select type="text" :name="field.name" class="form-control" v-model="selected_ward_id">
                ...
                <button type="submit" class="btn btn-primary btn-sm mt-3">Set Responsibility</button>
            </form>
        </div>
    </div>
</template>
...
export default {
    props: ['user_id','ward_id','responsibility_id'],
    components:{
        FormInput
    },
    data() {
    ...
    created(){
        ...
        this.selected_ward_id = this.ward_id;
    ...
    methods:{
        ...
        set_responsibility :async function(event){
            ...
            try {
                let response = await axios.post('/thana/ward-jonoshokti/set-responsibility', formData);
