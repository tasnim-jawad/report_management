    ...
    <section id="heading" class="mt-3">
        ...
        <div class="unit_info">
            ...
            <div class="line d-flex flex-wrap justify-content-between ">
                <p>ওয়ার্ডের সংখ্যা : {{ bangla($report_header?->ward_count) ?? '' }}
                <p>পৌরসভা/ইউনিয়ন: </p>
                <p class="width-30">উপজেলা/থানা: {{ $report_header?->thana_info?->title ?? '' }}</p>
            </div>
            <div class="line d-flex flex-wrap justify-content-between ">
                {{-- <p>আমীর/সভাপতির নাম : {{ $report_header->president ?? '' }}</p> --}}
                <p>ওয়ার্ডের সমুহের নাম :
                    @foreach ($report_header?->ward_titles as $ward_name)
                        <span class="">{{$ward_name}},</span>
                    @endforeach
                </p>
            </div>
        </div>
    ...
    <script>
        function print_upload_page(event) {
            ...
            if (user_id && month) {
                ...
                const redirectUrl = `/dashboard/ward#/ward-report-upload-monthly/${month}/${user_id}`;
        ...
        function go_back_to_dashboard(event) {
            ...
            if (user_id && month) {
                // Construct the new URL
                const redirectUrl = `/dashboard/thana#/dashboard`;
